...
import AddNewDocumentType from './modal/AddNewModal'
import EditDocumentType from './modal/EditModal'
...
const DocumentType = () => {
    ...
    const handleDeleteDocumentType = (data) => {
        return Swal.fire({
            title: '',
            text: 'Bạn có muốn xóa loại tài liệu này không?',
            icon: 'warning',
            ...
            if (result.value) {
                deleteDocumentType(data).then(result => {
                    if (result.status === 'success') {
                        Swal.fire({
                            icon: 'success',
                            title: 'Xóa loại tài liệu thành công!',
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'Xóa loại tài liệu thất bại!',
                ...
                return (
                    <div className="column-action d-flex align-items-center">
                        ...
                        {ability.can('delete', 'nguoidung') &&
                            <div id="tooltip_trash" style={{ marginRight: '1rem', marginLeft: '1rem' }} onClick={() => handleDeleteDocumentType(row.id)}>
                                <Trash
                                    size={15}
        ...
        <Fragment>
            <Card style={{ backgroundColor: 'white' }}>
                <CardHeader className='flex-md-row flex-column align-md-items-center align-items-start border-bottom'>
                    <CardTitle tag='h4'>Danh sách loại tài liệu</CardTitle>
            ...
            <AddNewDocumentType open={modalAddNew} handleAddModal={handleAddModal} getData={fetchDocumentType} />
            {dataEdit && <EditDocumentType open={modalEdit} handleEditModal={handleEditModal} getData={fetchDocumentType} dataEdit={dataEdit} />}
        </Fragment >
    )
}
