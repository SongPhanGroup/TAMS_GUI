...
import { getPermission, createPermission, deletePermission, updatePermission } from "../../../api/permission"
import { AbilityContext } from '@src/utility/context/Can'

const ListPermissions = ({ record }) => {
  ...
  const [permissionGroupID_, setPermissionGroupID] = useState(null)
        ...
        <div style={{ display: "flex", justifyContent: "space-around" }}>
          ...
          {ability.can('update', 'NHOM_QUYEN') &&
            <Popconfirm
              ...
              okText="Đồng ý"
            >
              <DeleteOutlined style={{ color: "red", cursor: 'pointer' }} id={`tooltip_delete${record._id}`} />
  ...
  return (
    <Card
      title={`Quyền chức năng nhóm ${record?.permissionGroupName}`}
    ...
    >
      ...
      <AddNewModal open={isAdd} handleModal={handleModal} getData={getData} rowsPerPage={rowsPerPage} currentPage={currentPage} listGroup={listGroup} setCurrentPage={setCurrentPage} infoEdit={record} />
      {
        <EditModal open={isEdit} handleModal={handleModal} getData={getData} infoEdit={info} listGroup={listGroup} currentPage={currentPage} setCurrentPage={setCurrentPage} rowsPerPage={rowsPerPage} record={record} />
      }
    </Card>
  )
}
